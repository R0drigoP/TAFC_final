...
def objective(trial):
	...
	m0 = trial.suggest_float('m0',0.001,0.6)
	...
	#compiling
	process = sub.Popen(["make"],shell =True);
	...
	else: #return nan nb of calls so that we dont study the convergence when we haven't achieved the global minimum 
		return 1e6
...
study.enqueue_trial({'N_molecules': 5, 'survival_rate': 0.8635528546962796, 'mutation_prob': 0.026222012776675394, 'm0': 0.08692457788173921, 'L_box': 0.6665222303702193})
study.enqueue_trial({'N_molecules': 173, 'survival_rate': 0.7962394409129764, 'mutation_prob': 0.0029140060146080845, 'm0': 0.1475360462237306, 'L_box': 0.6009989796083813})
...
#-----
fig = optuna.visualization.plot_parallel_coordinate(study, params=['L_box','m0'])
fig.write_image("plot_parallel_coordinate_L_m0.pdf")

fig = optuna.visualization.plot_parallel_coordinate(study, params=['L_box','mutation_prob'])
fig.write_image("plot_parallel_coordinate_L_mut.pdf")

fig = optuna.visualization.plot_parallel_coordinate(study, params=['L_box','N_molecules'])
fig.write_image("plot_parallel_coordinate_L_mol.pdf")


fig = optuna.visualization.plot_parallel_coordinate(study, params=['L_box','survival_rate'])
fig.write_image("plot_parallel_coordinate_L_surv.pdf")

fig = optuna.visualization.plot_parallel_coordinate(study, params=['N_molecules','m0'])
fig.write_image("plot_parallel_coordinate_mol_m0.pdf")

fig = optuna.visualization.plot_parallel_coordinate(study, params=['N_molecules','mutation_prob'])
fig.write_image("plot_parallel_coordinate_mol_mut.pdf")

fig = optuna.visualization.plot_parallel_coordinate(study, params=['N_molecules','survival_rate'])
fig.write_image("plot_parallel_coordinate_mol_surv.pdf")

fig = optuna.visualization.plot_parallel_coordinate(study, params=['m0','mutation_prob'])
fig.write_image("plot_parallel_coordinate_m0_mut.pdf")

fig = optuna.visualization.plot_parallel_coordinate(study, params=['m0','survival_rate'])
fig.write_image("plot_parallel_coordinate_m0_surv.pdf")

fig = optuna.visualization.plot_parallel_coordinate(study, params=['mutation_prob','survival_rate'])
fig.update_yaxes(type='log', title='Objective Value', selector=dict(name='value'))
fig.write_image("plot_parallel_coordinate_mut_surv.pdf")
